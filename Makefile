build:
	go build -o bin/gitlab-external-pipeline-trigger ./cmd

run: build
	./bin/gitlab-external-pipeline-trigger
...
untag:
	...
	curl --request DELETE --header "Authorization: token ${GITHUB_TOKEN}" "https://api.github.com/repos/slamdev/gitlab-external-pipeline-trigger/releases/:release_id/$(TAG)"
